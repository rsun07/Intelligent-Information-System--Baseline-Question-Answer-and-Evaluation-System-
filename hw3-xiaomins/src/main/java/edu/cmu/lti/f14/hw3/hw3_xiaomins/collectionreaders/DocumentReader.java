...
/**
 * The DocumentReader parses lines from the input document file and saves the result of the parse
 * (query id, relevance assessment, and document text) in a type Document Then through CAS, output
 * the information to the Annotator
 *
 * @author Ryan Sun
 */
public class DocumentReader extends JCasAnnotator_ImplBase {
  ...
  @Override
  /**
   * The process function afford the main task for the DocumentReader.
   * Read document from the input JCas, call the parseDataLine() function 
   * to parse query id, relevance and text.
   * Output the information to Annotator through JCas and Document type
   * 
   */
  ...
  /**
   * The parseDateLine function can read a input line and parse it into three parts, that's the
   * question Id, the relevance value and the text
   * 
   * @param line
   * @return
   */
